...
export default function FinancialSummary({ householdId, privateMode }) {
    ...
    const refresh = useCallback(async () => {
        ...
        setStatus("loading");
        try { setSummary(await request("/v1/financial/summary")); setAsOf(new Date()); setError(""); setStatus("ready"); }
    ...
    if (status === "loading") return <section className="financial-summary financial-summary-loading" aria-label="Loading 90-day financial summary"><Skeleton className="skeleton-hero" /></section>;
    if (status === "error") return <section className="financial-summary" aria-label="Financial summary unavailable"><div><span className="eyebrow">MONEY · LAST 90 DAYS</span><h2>Connect a bank or import a CSV.</h2><p>{error}</p><small>Bank deposits can take a few minutes to appear after Chime posts them.</small></div><button className="button secondary" type="button" onClick={refresh}><RefreshCw size={16} /> Try again</button></section>;

    const net = Number(summary?.net) || 0;
    return <section className="financial-summary" aria-labelledby="financial-summary-title">
        <header><div><span className="eyebrow">MONEY · LAST 90 DAYS</span><h2 id="financial-summary-title">{net >= 0 ? "+" : "−"}{showMoney(net)} net</h2><p>{summary?.transaction_count ? "A clear view of what came in and what went out." : "Connect a bank or import a CSV — it takes about two minutes."}</p><small>{asOf ? `As of ${asOf.toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })} · Bank deposits can take a few minutes to appear.` : null}</small></div><button className="icon-button" type="button" onClick={refresh} aria-label="Refresh money summary"><RefreshCw size={19} /></button></header>
